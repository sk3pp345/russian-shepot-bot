...
ADMINS = [1252647696] # Добавь сюда ID всех админов
...
async def cmd_check(message: types.Message, command: CommandObject):
    ...
    try:
        ...
        await message.answer(f"🔍 <b>Пост №{command.args}</b>\nАвтор: @{p['username']}\nID автора: <code>{p['user_id']}</code>\n\nТекст: {p['text']}", parse_mode="HTML")
    except: await message.answer("❌ Пост не найден.")
...
async def cmd_history(message: types.Message, command: CommandObject):
    ...
    user_id = next((uid for uid, un in db["users"].items() if un == uname), "Не найден")
    await message.answer(f"📝 <b>История @{uname}:</b>\nID: <code>{user_id}</code>\nПосты: {', '.join(user_posts) if user_posts else 'нет'}", parse_mode="HTML")

@dp.message(Command("send"), F.from_user.id.in_(ADMINS))
async def cmd_send(message: types.Message, command: CommandObject):
    if not command.args: return await message.answer("Введите текст: /send Всем привет!")
    db = load_db()
    users = db["users"].keys()
    count = 0
    for uid in users:
...
async def action_shop(message: types.Message):
    ...
    await message.answer("💎 <b>Магазин:</b>", reply_markup=kb, parse_mode="HTML")
...
async def shop_categories(call: types.CallbackQuery):
    ...
    await call.message.edit_text(f"🛍 <b>Категория: {cat}</b>", reply_markup=InlineKeyboardMarkup(inline_keyboard=kb_list), parse_mode="HTML")
...
async def process_buy(call: types.CallbackQuery):
    ...
    await bot.send_invoice(call.from_user.id, title=item, description=f"Оплата '{item}'", payload=f"pay_{item}", currency="XTR", prices=[LabeledPrice(label=item, amount=int(price))])
    await call.answer()
...
async def success_pay(m: types.Message):
    await m.answer("✅ Оплачено!")
    for aid in ADMINS: await bot.send_message(aid, f"💰 Оплата от @{m.from_user.username}: {m.successful_payment.invoice_payload}")
...
async def set_state(message: types.Message):
    ...
    save_db(db)
    await message.answer("Пришлите ваш контент или сообщение:", reply_markup=get_back_kb())
...
async def process_post(call: types.CallbackQuery):
    ...
    if act == "acc":
        ...
        await bot.send_message(int(post["user_id"]), "❌ Пост отклонен.")
...
async def support_reply_call(call: types.CallbackQuery):
    ...
    save_db(db)
    await call.message.answer(f"✍️ Пиши ответ для {uid}:")
    await call.answer()
